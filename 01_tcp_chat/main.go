...
func echoHandler(conns *map[string]net.Conn, messages chan string) {
	println("in echoHandler")

	for {
		println("in echoHandler, for__00")

		// 下面一句是阻塞的
		msg := <-messages
		println("in echoHandler, for__01, after msg:= <- messages")
		fmt.Println(msg)
		...
		println("in echoHandler, for__99, after msg:= <- messages")
	}
	println("in echoHandler end")
}
